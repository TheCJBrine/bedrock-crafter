...
public class BedrockPickaxeItem extends PickaxeItem {
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        assert player != null;
        BlockPos voidPos = new BlockPos(player.getBlockPos().getX(), worldBottom, player.getBlockPos().getZ());
        ...
        if(blockState.isIn(BCTags.BCBlockTags.BEDROCK_LIKE)) {
            if(itemStack.getDamage() <= itemStack.getMaxDamage() - 100) {
                if(worldHasCeiling && blockPos.getY() > worldBottom && (blockPos.getY() < world.getDimension().getLogicalHeight() - 1 ||
                    ...
                    return ActionResult.SUCCESS;
                } else if(blockPos.getY() <= worldBottom || blockPos.getY() == world.getDimension().getLogicalHeight() - 1) {
