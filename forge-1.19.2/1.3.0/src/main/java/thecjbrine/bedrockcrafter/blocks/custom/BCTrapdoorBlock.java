...
public class BCTrapdoorBlock extends TrapDoorBlock {
    ...
    protected void playSound(@Nullable Player player, Level level, BlockPos pos, boolean open) {
        if(open) {
            level.playSound(player, pos, BCSoundEvents.BLOCK_BEDROCK_TRAPDOOR_OPEN.get(), SoundSource.BLOCKS, 1F, 1F);
        } else {
            level.playSound(player, pos, BCSoundEvents.BLOCK_BEDROCK_TRAPDOOR_CLOSE.get(), SoundSource.BLOCKS, 1F, 1F);
        }
