...
public class BedrockPickaxeItem extends PickaxeItem {
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        assert player != null;
        BlockPos voidPos = new BlockPos(player.getBlockX(), world.getDimension().getMinimumY(), player.getBlockZ());
        ...
        if(blockState.isIn(BCTags.BCBlockTags.BEDROCK_LIKE)) {
            if(itemStack.getDamage() <= itemStack.getMaxDamage() - 100) {
                if(worldHasCeiling && blockPos.getY() > world.getDimension().getMinimumY() && (blockPos.getY() < world.getDimension().getLogicalHeight() - 1 ||
                        blockPos.getY() > world.getDimension().getLogicalHeight() - 1)) {
                    magicallyMineBedrock(context);

                    return ActionResult.SUCCESS;
                } else if(worldIsFloating && blockPos != voidPos) {
                    magicallyMineBedrock(context);

                    return ActionResult.SUCCESS;
                } else if(blockPos.getY() > world.getDimension().getMinimumY() && !worldHasCeiling) {
                    magicallyMineBedrock(context);

                    return ActionResult.SUCCESS;
                } else if(blockPos.getY() <= world.getDimension().getMinimumY() || blockPos.getY() >= world.getDimension().getLogicalHeight() - 1) {
