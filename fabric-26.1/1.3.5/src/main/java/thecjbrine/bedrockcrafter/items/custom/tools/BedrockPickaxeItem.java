...
public class BedrockPickaxeItem extends Item {
    ...
    public BedrockPickaxeItem(ToolMaterial material, float attackDamage, float attackSpeed, Properties properties) {
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        assert player != null;
        BlockPos voidPos = new BlockPos(player.getBlockX(), level.dimensionType().minY(), player.getBlockZ());
        boolean worldHasCeiling = level.dimensionType().hasCeiling();
        boolean worldIsFloating = !level.getBlockState(voidPos).is(BCTags.BCBlockTags.BEDROCK_LIKE);
        ...
        if(blockState.is(BCTags.BCBlockTags.BEDROCK_LIKE)) {
            if(itemStack.getDamageValue() <= itemStack.getMaxDamage() - 100) {
                if(worldHasCeiling && blockPos.getY() > level.dimensionType().minY() && (blockPos.getY() < level.dimensionType().logicalHeight() - 1 ||
                    ...
                    return InteractionResult.SUCCESS;
                } else if(blockPos.getY() > level.dimensionType().minY() && !worldHasCeiling) {
                    magicallyMineBedrock(context);
    ...
    private void magicallyMineBedrock(UseOnContext context) {
        Player player = context.getPlayer();
        Level level = context.getLevel();
        BlockPos blockPos = context.getClickedPos();
        BlockState blockState = level.getBlockState(blockPos);
        ...
        level.playSound(player, blockPos, BCSoundEvents.ITEM_BEDROCK_PICKAXE_USE, SoundSource.BLOCKS, 1F, 1F);

        if(player != null && !player.isCreative()) {
            player.addItem(new ItemStack(blockState.getBlock().asItem()));
            if(itemStack.getDamageValue() < itemStack.getMaxDamage() - 101) {
                itemStack.hurtAndBreak(100, player, context.getHand().asEquipmentSlot());
